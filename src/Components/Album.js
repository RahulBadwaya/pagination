...
const Album =()=>{
 ...
 const prevPage =()=>{
   ...
   console.log(start,end)
    ...
    const jump =(value)=>{
      ...
      const updatedImgs = data.filter(img=> (img.id >= (newStart + 1)  && img.id<= newStart + LIMIT));
   setImgs(updatedImgs)
    }
 ...
 <div align="center">
    ...
    {imgs.map((ob,i)=>{
      if(i<10)
      return <button className="btn-lg btn-outline-info btn-bottom" onClick={() => jump(i)}>{i+1}</button>
    })}
    &nbsp;&nbsp;&nbsp;
